import { JWT, JwtPayload, JwtOptions } from "./JWT";

describe("JWT", () => {
  const secretKey = "una-clave-secreta-muy-larga-y-compleja";
  const defaultPayload: JwtPayload = { userId: 123, role: "admin" };
  let jwt: JWT;
  ...
  describe("generate", () => {
    ...
    it("should throw an error if payload is invalid", () => {
      expect(() => jwt.generate(null as any)).toThrow("Error generando JWT");
    });
  });
  ...
  describe("verify", () => {
    ...
    it("should return null for expired token", () => {
      ...
      (Date.now as jest.Mock).mockReturnValue(1750021744000 + 61 * 1000);
